...
flowchart TD
    ...
    H --> J[Analyzer — Claude API]
    I --> J
    J --> K{Валидный JSON?}
    K -- Нет --> L[Retry до 3 раз]
    L --> K
    K -- Да --> M[Показать результат\nСписать кредит]
    K -- Ошибка API --> N[Кредит не списан\nСообщение об ошибке]
    M --> O{Оценка пользователя}
    O -- 4-5 --> P([Готово])
    O -- 0-3 --> Q[Запрос комментария\n+ согласие на хранение резюме]
    Q --> R[Сохранить для улучшения промпта]
    R --> P
```
